...
class TodoCalendarViewController: UIViewController {
    ...
    private var button: UIButton = {
        ...
        button.setImage(ImagesUIKit.plus, for: .normal)
...
private extension TodoCalendarViewController {
    ...
    func configureTableView() {
        tableView.register(TodoCalendarTableViewCell.self, 
                           forCellReuseIdentifier: "TodoCalendarTableViewCell")
    ...
    func configureCollectionView() {
        collectionViewWithDates.register(TodoCalendarCell.self, 
                                         forCellWithReuseIdentifier: "TodoCalendarCell")
    ...
    func configureButton() {
        ...
        button.addTarget(self, action: #selector(didAddNewButtonTapped), 
                         for: .touchUpInside)
    }
}
...
private extension TodoCalendarViewController {
    ...
    func openTodoItemView() {
        let todoItemViewModel = TodoItemViewModel(todoItem: nil, 
                                                  listViewModel: TodoListViewModel(fileCache: todocalendarViewModel.fileCache))
        let todoItemView = TodoItemViewWrapper(isShowed: Binding(get: 
                                                                    { self.isShowTodoItemView },
                                                                 set:
                                                                    { self.isShowTodoItemView = $0 }), 
